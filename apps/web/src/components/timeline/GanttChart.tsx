    ...
    <div className="border border-slate-200 rounded-md overflow-hidden bg-white">
      <div className="flex">
        ...
        <div className="flex-1 min-w-0">
          <TimeScaleHeader scales={scales} startDate={startDate} totalDurationWeeks={total} />
        </div>
      </div>
        ...
        return (
          <div key={milestone.milestoneId}>
            <div className="flex bg-slate-100 border-t border-b border-slate-200">
              <div className="w-56 flex-shrink-0 px-3 py-1 text-xs font-semibold text-slate-600">{milestone.name}</div>
                ...
                <div className="flex-1 min-w-0 relative h-8">
                  {row.segments.map((seg, i) => (
                    <div
                      ...
                      style={{
                        left: `${(seg.startOffsetWeeks / total) * 100}%`,
                        width: `${Math.max((seg.durationWeeks / total) * 100, 0.3)}%`,
                      }}
